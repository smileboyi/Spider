...
class QSBK:
  ...
  def getPageItems(self,pageIndex):
    ...
    pattern = re.compile('<div.*?author clearfix">.*?<a.*?<h2>(.*?)</h2>.*?<div.*?content">'+
                          '.*?<span>(.*?)</span>.*?</div>.*?</a>(.*?)<div class="stats.*?class="number">(.*?)</i>',re.S)
